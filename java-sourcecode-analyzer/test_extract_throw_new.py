#! /usr/bin/python
#coding: utf-8
#filename test_extract_throw_new.py
import os
import os.path
import datetime
import sys
...
all_exception_class_dict=defaultdict(int)

def stats_exception_class( start_path ):
    for file_name in getNextJavaFile( start_path ):
        ...
        for obj in func_list:
            ...
            for line in obj.ori_code_list:
                if isJavaThrowNewLine( line ):
                    ...
                    #if all_exception_class_dict.get( tmp_throw_new_cls ) is None:
                    #all_exception_class_dict[ tmp_throw_new_cls ]=tmp_throw_new_cls
                    all_exception_class_dict[ tmp_throw_new_cls ] += 1
    for item in all_exception_class_dict:
        print all_exception_class_dict[item],"\t,",item
...
if True:
    ...
    '''
    LOG_FILENAME = '/home/es_catch_exceptions2.log'    
    logging.basicConfig(filename=LOG_FILENAME,level=logging.DEBUG,)
    if "__main__" == __name__ :  
        ...
        if "default" != options.java_path:
            stats_exception_class( options.java_path )
